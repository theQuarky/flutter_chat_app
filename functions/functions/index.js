...
const serviceAccount = {
  ...
  client_x509_cert_url:
    "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-lv5ea%40flutter-c91c8.iam.gserviceaccount.com",
...
exports.sendNotification = functions.https.onCall((data, context) => {
  ...
  if (_.isEmpty(params.body)) {
    return response.status(400).send("Body is required");
  }
  ...
  messaging
    ...
    .then((result) => {
      ...
      return response.status(200).send({ message: "Notification send" });
    })
    .catch((err) => {
      console.log("Err: ", err);
      return response
        .status(400)
        .send({ message: "Error occured while sending notificaiton" });
    });
});
